...
import {stories,processFigures} from '../src/lesson-story.js';
...
export async function testLessons(browser,base){
 ...
 const evidence={base,chapters:[],roundtrips:[],figures:[],axe:[],checks:0,projectChecks:0,processFigures:[]};
 try{
  ...
  assert.deepEqual(coverage.narrative,topics.map(t=>({topic:t.id,sections:4,transfers:8,checkpoint:1})));
  assert.deepEqual(coverage.processFigures,Object.keys(processFigures).map(topic=>({id:`${topic}-prozess`,topic})));
  ...
  for(const topic of topics){
   ...
   assert.equal(await page.locator('.reasoning-step').count(),4);
   assert.equal(await page.locator('.story-transfer').count(),8);
   assert.equal(await page.locator('.story-notice').count(),1);
   for(const t of ts)assert.equal(await page.locator(`#${t.id} .story-transfer p`).textContent(),transfers[t.id]);
   const pause=page.locator('#projektcheck details');await pause.locator('summary').focus();await pause.locator('summary').press('Enter');assert.equal(await pause.getAttribute('open'),'');assert.equal(await pause.locator('p').textContent(),stories[topic.id].checkpoint[1]);await pause.locator('summary').press('Enter');evidence.projectChecks++;
   ...
   if(processFigures[topic.id]){
    const diagram=page.locator(`#${topic.id}-prozess`);
    for(const width of [1440,360]){await page.setViewportSize({width,height:1000});await diagram.screenshot({path:`artifacts/chapters/${topic.id}-process-${width}.png`});}
    evidence.processFigures.push(topic.id);await page.setViewportSize({width:1440,height:1000});
   }
   await page.locator('#verstehen').screenshot({path:`artifacts/chapters/${topic.id}-reasoning.png`});
  ...
  assert.equal(evidence.roundtrips.length,72);assert.equal(evidence.figures.length,10);assert.equal(evidence.checks,27);assert.equal(evidence.projectChecks,9);assert.equal(evidence.processFigures.length,3);
  ...
  assert.ok(!requests.some(url=>/\/(chapters|lesson-examples|lesson-story|lesson-transfer)\.js/.test(url)),'Keine Lehrtextdaten im Laufzeitbundle');
  ...
  const offline=await browser.newContext({javaScriptEnabled:false});const staticPage=await offline.newPage();for(const t of topics){await staticPage.goto(new URL(`lernen/${t.id}.html`,base).href);assert.equal(await staticPage.locator('.worked').count(),8);assert.equal(await staticPage.locator('.story-transfer').count(),8);await staticPage.locator('#projektcheck summary').click();assert.equal(await staticPage.locator('#projektcheck details').getAttribute('open'),'');assert.ok(await staticPage.locator('svg[role="img"]').count());await staticPage.locator('#selbstcheck summary').first().click();assert.equal(await staticPage.locator('#selbstcheck details').first().getAttribute('open'),'');}await offline.close();
  assert.deepEqual(errors,[]);await writeFile('artifacts/lesson-browser-report.json',JSON.stringify(evidence,null,2));console.log(`LESSON BROWSER PASS: 9 Kapitel, 72 geklickte Hin-/Rückwege, 10 interaktive SVG (Tastatur und Zahlen), 27 Selbstchecks + 9 Projektchecks, 3 Vergleichsbilder, 18 axe-Scans, 4 Breiten, Legacy-/gesperrter Speicher, ohne JavaScript. Ziel: ${base}`);
