...
export function visual(q){
 if(q.triangle){const {a,b}=q.triangle,s=210/Math.max(a,b),x=75,y=250,w=a*s,h=b*s;return wrap(`<path d="M${x} ${y-h}V${y}H${x+w}Z" fill="#eae6ff" stroke="#3930a3" stroke-width="3"/><path d="M${x} ${y-16}h16v16" fill="none" stroke="#3930a3"/><text x="260" y="277" text-anchor="middle">Ankathete / waagerechte Kathete</text><text x="${x+20}" y="${y-h-12}">A</text><text x="${x+w+12}" y="${y}">α</text>`,`Maßstäbliches rechtwinkliges Dreieck. Waagerechte und senkrechte Kathete im Verhältnis ${f(a,2)} zu ${f(b,2)}. α liegt rechts unten; die Hypotenuse ist die schräge Seite.`);}
 if(q.bars){const {values,base}=q.bars,max=Math.max(...values)-base;let b=`<path d="M70 25V250H470" fill="none" stroke="#68685d"/><text x="12" y="255">${base}</text><text x="12" y="35">${Math.max(...values)}</text>`;values.forEach((v,i)=>{let h=(v-base)/max*215;b+=`<rect x="${140+i*170}" y="${250-h}" width="80" height="${h}" fill="#3930a3"/><text x="${180+i*170}" y="${240-h}" text-anchor="middle">${v}</text><text x="${180+i*170}" y="275" text-anchor="middle">${i+1}</text>`;});return wrap(b,`Achtung: verkürzte y-Achse ab ${base}. Die beiden Datenwerte sind ${values.join(' und ')}. Balken beginnen hier nicht bei null.`);}
 if(!q.graph)return '';
 ...
 let body='';for(let i=0;i<=4;i++){const x=xmin+(xmax-xmin)*i/4,y=ymin+(ymax-ymin)*i/4;body+=`<path d="M${X(x)} 35V250 M65 ${Y(y)}H485" stroke="#deded5" stroke-width="1"/><text x="${X(x)}" y="273" text-anchor="middle">${f(x,1)}</text><text x="57" y="${Y(y)+5}" text-anchor="end">${f(y,1)}</text>`;}
 body+=`<path d="M65 ${Y(0)}H485" stroke="#68685d"/>`;
 if(xmin<=0&&xmax>=0)body+=`<path d="M${X(0)} 35V250" stroke="#68685d"/>`;
 if(pointsOnly){for(let x=0;x<=8;x++)body+=`<circle cx="${X(x)}" cy="${Y(fn(x))}" r="4" fill="#3930a3"/>`;}
 else body+=`<polyline points="${samples.map(([x,y])=>`${X(x)},${Y(y)}`).join(' ')}" fill="none" stroke="#3930a3" stroke-width="3"/>`;
